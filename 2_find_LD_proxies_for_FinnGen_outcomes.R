...
out_func_FINNGEN <- function(file, name)
{
  ...
  outcome_var <- read.table(paste("folder_containing_finngen_data/finngen_", file,".txt", sep = ""), 
...
{
  # Extract outcome SNPs matching the SNPs in the exposure dataset
  outcome_var <- read.table(paste("data/FINNGEN/finngen_", file,"_inc_proxies.txt", sep = ""), 
